...
SwitchWatcher::SwitchWatcher(AllJoynBusAttachment^ busAttachment) :
...
void SwitchWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.microsoft.ZWaveBridge.SwitchBinary.Switch" };
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.microsoft.ZWaveBridge.SwitchBinary.Switch"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.microsoft.ZWaveBridge.SwitchBinary.Switch"),
            port);
        Added(this, args);
    }
...
void SwitchWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<SwitchWatcher>
        };
...
void SwitchWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_SwitchIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &SwitchWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void SwitchWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void SwitchWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
