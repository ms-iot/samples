...
IAsyncOperation<interface_1GetContactResult^>^ interface_1Consumer::GetContactAsync()
{
    return create_async([this]() -> interface_1GetContactResult^
    {
        ...
        result->Contact = getContext.GetValue();
        return result;
    });
}
...
void interface_1Consumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    ...
    for (size_t i = 0; i < changedPropertyCount; i++)
    {
        ...
        if (strcmp("Contact", propertyName) == 0)
        {
            Platform::String^ argument;
            (void)TypeConversionHelpers::GetAllJoynMessageArg(propertyValue, "s", &argument);
            ContactChanged(this, (Platform::Object^)argument);
        }
    }
}
...
int32 interface_1Consumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo)
{
    ...
    alljoyn_interfacedescription description = alljoyn_busattachment_getinterface(m_nativeBusAttachment, "com.enocean.Bridge.WindowContact.interface_1");
