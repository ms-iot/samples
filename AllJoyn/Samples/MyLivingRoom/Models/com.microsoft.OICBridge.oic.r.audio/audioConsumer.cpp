...
std::map<alljoyn_interfacedescription, WeakReference*> audioConsumer::SourceInterfaces;

audioConsumer::audioConsumer(AllJoynBusAttachment^ busAttachment)
...
void audioConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void audioConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void audioConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus audioConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<audioJoinSessionResult^>^ audioConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ audioWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> audioJoinSessionResult^
    {
        auto result = ref new audioJoinSessionResult();
        result->Status = AllJoynStatus::Ok;
        result->Consumer = nullptr;

        result->Consumer = ref new audioConsumer(watcher->BusAttachment);
...
IAsyncOperation<audioSetMuteResult^>^ audioConsumer::SetMuteAsync(_In_ bool value)
{
    return create_async([this, value]() -> audioSetMuteResult^
    {
...
IAsyncOperation<audioGetMuteResult^>^ audioConsumer::GetMuteAsync()
{
    return create_async([this]() -> audioGetMuteResult^
    {
...
IAsyncOperation<audioSetVolumeResult^>^ audioConsumer::SetVolumeAsync(_In_ int64 value)
{
    return create_async([this, value]() -> audioSetVolumeResult^
    {
        PropertySetContext setContext;
...
IAsyncOperation<audioGetVolumeResult^>^ audioConsumer::GetVolumeAsync()
{
    return create_async([this]() -> audioGetVolumeResult^
    {
...
void audioConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
    ...
    for (size_t i = 0; i < changedPropertyCount; i++)
    {
        ...
        if (strcmp("Mute", propertyName) == 0)
        {
            MuteChanged(this, nullptr);
        }
...
int32 audioConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo, const std::string& ifName)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<audioConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<audioConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<audioConsumer>
    };
    ...
    PCSTR propertyNames[] = { "Mute", "Volume" };
    ...
    RETURN_IF_QSTATUS_ERROR(alljoyn_proxybusobject_registerpropertieschangedlistener(
        ...
        AllJoynHelpers::PropertyChangedHandler<audioConsumer>, 
        m_weak));
