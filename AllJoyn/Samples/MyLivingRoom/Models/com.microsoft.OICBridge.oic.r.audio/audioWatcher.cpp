...
audioWatcher::audioWatcher(AllJoynBusAttachment^ busAttachment) :
...
void audioWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.microsoft.OICBridge.*oic.r.audio" };
                        ...
                        if (0 < nbOfInterfaces)
                        {
                            ...
                            if (nullptr != arrayOfInterfaces)
                            {
                                if (alljoyn_aboutobjectdescription_getinterfaces(objectDescription, arrayOfPath.get()[iPath], arrayOfInterfaces.get(), nbOfInterfaces) > 0)
                                {
                                    ...
                                    {
                                        if (string(arrayOfInterfaces.get()[iBusInterface]).find("oic.r.audio") != string::npos)
                                        {
...
void audioWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<audioWatcher>
        };
...
void audioWatcher::Start()
{
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &audioWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void audioWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void audioWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
