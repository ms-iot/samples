...
airflowWatcher::airflowWatcher(AllJoynBusAttachment^ busAttachment) :
...
airflowWatcher::~airflowWatcher()
{
    UnregisterFromBus();
}

void airflowWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.microsoft.OICBridge.*oic.r.airflow" };
...
void airflowWatcher::OnAnnounce(
    _In_ PCSTR name,
                        ...
                        if (0 < nbOfInterfaces)
                        {
                            ...
                            if (nullptr != arrayOfInterfaces)
                            {
                                if (alljoyn_aboutobjectdescription_getinterfaces(objectDescription, arrayOfPath.get()[iPath], arrayOfInterfaces.get(), nbOfInterfaces) > 0)
                                {
                                    ...
                                    {
                                        if (string(arrayOfInterfaces.get()[iBusInterface]).find("oic.r.airflow") != string::npos)
                                        {
...
void airflowWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<airflowWatcher>
        };
...
void airflowWatcher::Start()
{
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &airflowWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void airflowWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void airflowWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
