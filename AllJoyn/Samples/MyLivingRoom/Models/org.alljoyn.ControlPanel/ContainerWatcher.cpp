...
ContainerWatcher::ContainerWatcher(AllJoynBusAttachment^ busAttachment) :
...
void ContainerWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "org.alljoyn.ControlPanel.Container" };
...
void ContainerWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "org.alljoyn.ControlPanel.Container"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "org.alljoyn.ControlPanel.Container"),
            port);
        Added(this, args);
    }
...
void ContainerWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<ContainerWatcher>
        };
...
void ContainerWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_ContainerIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &ContainerWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void ContainerWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void ContainerWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
