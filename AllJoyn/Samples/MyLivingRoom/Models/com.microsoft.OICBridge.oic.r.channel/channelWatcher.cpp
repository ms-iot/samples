...
channelWatcher::channelWatcher(AllJoynBusAttachment^ busAttachment) :
...
void channelWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.microsoft.OICBridge.oic.r.channel" };
...
void channelWatcher::OnAnnounce(
    _In_ PCSTR name,
                        ...
                        if (0 < nbOfInterfaces)
                        {
                            ...
                            if (nullptr != arrayOfInterfaces)
                            {
                                if (alljoyn_aboutobjectdescription_getinterfaces(objectDescription, arrayOfPath.get()[iPath], arrayOfInterfaces.get(), nbOfInterfaces) > 0)
                                {
                                    ...
                                    {
                                        if (string(arrayOfInterfaces.get()[iBusInterface]).find("oic.r.channel") != string::npos)
                                        {
...
void channelWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<channelWatcher>
        };
...
void channelWatcher::Start()
{
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &channelWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void channelWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void channelWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
