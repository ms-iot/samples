...
MCUWatcher::MCUWatcher(AllJoynBusAttachment^ busAttachment) :
...
void MCUWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "net.allplay.MCU" };
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "net.allplay.MCU"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "net.allplay.MCU"),
            port);
        Added(this, args);
    }
...
void MCUWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<MCUWatcher>
        };
...
void MCUWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_MCUIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &MCUWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void MCUWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void MCUWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
