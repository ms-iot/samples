...
std::map<alljoyn_busobject, WeakReference*> brightnessProducer::SourceObjects;
std::map<alljoyn_interfacedescription, WeakReference*> brightnessProducer::SourceInterfaces;

brightnessProducer::brightnessProducer(AllJoynBusAttachment^ busAttachment)
...
brightnessProducer::~brightnessProducer()
{
    UnregisterFromBus();
    delete m_weak;
}

void brightnessProducer::UnregisterFromBus()
{
...
bool brightnessProducer::OnAcceptSessionJoiner(_In_ alljoyn_sessionport sessionPort, _In_ PCSTR joiner, _In_ const alljoyn_sessionopts opts)
{
...
void brightnessProducer::OnSessionJoined(_In_ alljoyn_sessionport sessionPort, _In_ alljoyn_sessionid id, _In_ PCSTR joiner)
{
    ...
    {
        AllJoynHelpers::SessionLostHandler<brightnessProducer>,
        AllJoynHelpers::SessionMemberAddedHandler<brightnessProducer>,
        AllJoynHelpers::SessionMemberRemovedHandler<brightnessProducer>
    };
...
void brightnessProducer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void brightnessProducer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void brightnessProducer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void brightnessProducer::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {   
        QStatus result = AllJoynHelpers::CreateProducerSession<brightnessProducer>(m_busAttachment, m_weak);
        if (ER_OK != result)
        {
            StopInternal(result);
            return;
...
QStatus brightnessProducer::AddMethodHandler(_In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_methodhandler_ptr handler)
{
...
QStatus brightnessProducer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
    alljoyn_interfacedescription_member member;
    if (!alljoyn_interfacedescription_getmember(interfaceDescription, methodName, &member))
    {
...
QStatus brightnessProducer::OnPropertyGet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _Inout_ alljoyn_msgarg value)
{
    ...
    if (0 == strcmp(propertyName, "Value"))
    {
        ...
        return create_task([](){}).then([=]() -> QStatus
        {
            ...
            return static_cast<QStatus>(TypeConversionHelpers::SetAllJoynMessageArg(value, "x", result->Value));
...
QStatus brightnessProducer::OnPropertySet(_In_ PCSTR interfaceName, _In_ PCSTR propertyName, _In_ alljoyn_msgarg value)
{
    UNREFERENCED_PARAMETER(interfaceName);
...
void brightnessProducer::EmitValueChanged()
{
    create_task([&]
    {
        alljoyn_msgarg value = alljoyn_msgarg_create();
        OnPropertyGet("com.microsoft.OICBridge.oic.r.light.brightness", "Value", value);

        alljoyn_busobject_emitpropertychanged(
            m_busObject,
            "com.microsoft.OICBridge.oic.r.light.brightness",
            "Value",
            value,
            m_sessionId);
...
void brightnessProducer::Start()
{
    ...
    QStatus result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_brightnessIntrospectionXml);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    result = AllJoynHelpers::CreateBusObject<brightnessProducer>(m_weak);
    if (result != ER_OK)
    {
        StopInternal(result);
        return;
    ...
    alljoyn_interfacedescription interfaceDescription = alljoyn_busattachment_getinterface(AllJoynHelpers::GetInternalBusAttachment(m_busAttachment), "com.microsoft.OICBridge.oic.r.light.brightness");
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^,AllJoynBusAttachmentStateChangedEventArgs^>(this, &brightnessProducer::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void brightnessProducer::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void brightnessProducer::StopInternal(int32 status)
{
    UnregisterFromBus();
    Stopped(this, ref new AllJoynProducerStoppedEventArgs(status));
}

int32 brightnessProducer::RemoveMemberFromSession(_In_ String^ uniqueName)
{
...
PCSTR com::microsoft::OICBridge::oic::r::light::brightness::c_brightnessIntrospectionXml = "<interface name=\"com.microsoft.OICBridge.oic.r.light.brightness\">"
"  <property name=\"Value\" type=\"x\" access=\"readwrite\">"
