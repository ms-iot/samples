...
brightnessWatcher::brightnessWatcher(AllJoynBusAttachment^ busAttachment) :
...
void brightnessWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.microsoft.OICBridge.*oic.r.light.brightness" };
...
void brightnessWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    /*if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.microsoft.OICBridge.oic.r.light.brightness"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.microsoft.OICBridge.oic.r.light.brightness"),
            port);
        Added(this, args);
    }*/
                        ...
                        if (0 < nbOfInterfaces)
                        {
                            ...
                            if (nullptr != arrayOfInterfaces)
                            {
                                if (alljoyn_aboutobjectdescription_getinterfaces(objectDescription, arrayOfPath.get()[iPath], arrayOfInterfaces.get(), nbOfInterfaces) > 0)
                                {
                                    ...
                                    {
                                        if (string(arrayOfInterfaces.get()[iBusInterface]).find("oic.r.light.brightness") != string::npos)
                                        {
                                            AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
                                                AllJoynHelpers::MultibyteToPlatformString(name),
...
void brightnessWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<brightnessWatcher>
        };
...
void brightnessWatcher::Start()
{
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &brightnessWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void brightnessWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void brightnessWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
