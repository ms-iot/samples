...
std::map<alljoyn_interfacedescription, WeakReference*> modeConsumer::SourceInterfaces;

modeConsumer::modeConsumer(AllJoynBusAttachment^ busAttachment)
...
void modeConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void modeConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void modeConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus modeConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<modeJoinSessionResult^>^ modeConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ modeWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> modeJoinSessionResult^
    {
        auto result = ref new modeJoinSessionResult();
        result->Status = AllJoynStatus::Ok;
        result->Consumer = nullptr;

        result->Consumer = ref new modeConsumer(watcher->BusAttachment);
...
IAsyncOperation<modeSetModesResult^>^ modeConsumer::SetModesAsync(_In_ Platform::String^ value)
{
    return create_async([this, value]() -> modeSetModesResult^
    {
...
IAsyncOperation<modeGetModesResult^>^ modeConsumer::GetModesAsync()
{
    return create_async([this]() -> modeGetModesResult^
    {
        ...
        result->Modes = getContext.GetValue();
        return result;
    });
}

IAsyncOperation<modeSetSupportedModesResult^>^ modeConsumer::SetSupportedModesAsync(_In_ Platform::String^ value)
{
    return create_async([this, value]() -> modeSetSupportedModesResult^
    {
        PropertySetContext setContext;
...
IAsyncOperation<modeGetSupportedModesResult^>^ modeConsumer::GetSupportedModesAsync()
{
    return create_async([this]() -> modeGetSupportedModesResult^
    {
        PropertyGetContext<Platform::String^> getContext;
        ...
        result->Status = getContext.GetStatus();
        result->SupportedModes = getContext.GetValue();
        return result;
    });
}

void modeConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
    ...
    for (size_t i = 0; i < changedPropertyCount; i++)
    {
        ...
        if (strcmp("Modes", propertyName) == 0)
        {
            ModesChanged(this, nullptr);
        }
        if (strcmp("SupportedModes", propertyName) == 0)
        {
            SupportedModesChanged(this, nullptr);
        }
    }
}

int32 modeConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo, const std::string& ifName)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<modeConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<modeConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<modeConsumer>
    };
    ...
    PCSTR propertyNames[] = { "Modes", "SupportedModes" };
    ...
    RETURN_IF_QSTATUS_ERROR(alljoyn_proxybusobject_registerpropertieschangedlistener(
        ...
        AllJoynHelpers::PropertyChangedHandler<modeConsumer>, 
        m_weak));
