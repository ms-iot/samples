...
engineWatcher::engineWatcher(AllJoynBusAttachment^ busAttachment) :
...
engineWatcher::~engineWatcher()
{
    UnregisterFromBus();
}

void engineWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.microsoft.OICBridge.core.rule.engine" };
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.microsoft.OICBridge.core.rule.engine"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.microsoft.OICBridge.core.rule.engine"),
            port);
        Added(this, args);
    }
...
void engineWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<engineWatcher>
        };
...
void engineWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_engineIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &engineWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void engineWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void engineWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
