...
std::map<alljoyn_interfacedescription, WeakReference*> binaryConsumer::SourceInterfaces;

binaryConsumer::binaryConsumer(AllJoynBusAttachment^ busAttachment)
...
void binaryConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void binaryConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void binaryConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus binaryConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<binaryJoinSessionResult^>^ binaryConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ binaryWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> binaryJoinSessionResult^
    {
        auto result = ref new binaryJoinSessionResult();
        result->Status = AllJoynStatus::Ok;
        result->Consumer = nullptr;

        result->Consumer = ref new binaryConsumer(watcher->BusAttachment);
...
IAsyncOperation<binarySetValueResult^>^ binaryConsumer::SetValueAsync(_In_ bool value)
{
    return create_async([this, value]() -> binarySetValueResult^
    {
...
IAsyncOperation<binaryGetValueResult^>^ binaryConsumer::GetValueAsync()
{
    return create_async([this]() -> binaryGetValueResult^
    {
...
void binaryConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
...
int32 binaryConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo, const std::string& ifName)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<binaryConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<binaryConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<binaryConsumer>
    };
    ...
    RETURN_IF_QSTATUS_ERROR(alljoyn_proxybusobject_registerpropertieschangedlistener(
        ...
        AllJoynHelpers::PropertyChangedHandler<binaryConsumer>, 
        m_weak));
