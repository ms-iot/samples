...
std::map<alljoyn_interfacedescription, WeakReference*> openlevelConsumer::SourceInterfaces;

openlevelConsumer::openlevelConsumer(AllJoynBusAttachment^ busAttachment)
...
openlevelConsumer::~openlevelConsumer()
{
...
void openlevelConsumer::OnSessionLost(_In_ alljoyn_sessionid sessionId, _In_ alljoyn_sessionlostreason reason)
{
...
void openlevelConsumer::OnSessionMemberAdded(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
void openlevelConsumer::OnSessionMemberRemoved(_In_ alljoyn_sessionid sessionId, _In_ PCSTR uniqueName)
{
    if (sessionId == m_sessionId)
    {
...
QStatus openlevelConsumer::AddSignalHandler(_In_ alljoyn_busattachment busAttachment, _In_ alljoyn_interfacedescription interfaceDescription, _In_ PCSTR methodName, _In_ alljoyn_messagereceiver_signalhandler_ptr handler)
{
...
IAsyncOperation<openlevelJoinSessionResult^>^ openlevelConsumer::JoinSessionAsync(
    _In_ AllJoynServiceInfo^ serviceInfo, _Inout_ openlevelWatcher^ watcher)
{
    return create_async([serviceInfo, watcher]() -> openlevelJoinSessionResult^
    {
        auto result = ref new openlevelJoinSessionResult();
        result->Consumer = ref new openlevelConsumer(watcher->BusAttachment);
...
IAsyncOperation<openlevelSetOpenLevelResult^>^ openlevelConsumer::SetOpenLevelAsync(_In_ int64 value)
{
    return create_async([this, value]() -> openlevelSetOpenLevelResult^
    {
        ...
        if (ER_OK == status)
        {
            return openlevelSetOpenLevelResult::CreateSuccessResult();
        }
        return openlevelSetOpenLevelResult::CreateFailureResult(static_cast<int>(status));
    });
}

IAsyncOperation<openlevelGetOpenLevelResult^>^ openlevelConsumer::GetOpenLevelAsync()
{
    return create_async([this]() -> openlevelGetOpenLevelResult^
    {
        ...
        result->OpenLevel = getContext.GetValue();
        return result;
    });
}

void openlevelConsumer::OnPropertyChanged(_In_ alljoyn_proxybusobject obj, _In_ PCSTR interfaceName, _In_ const alljoyn_msgarg changed, _In_ const alljoyn_msgarg invalidated)
{
    UNREFERENCED_PARAMETER(obj);
    ...
    for (size_t i = 0; i < changedPropertyCount; i++)
    {
        ...
        if (strcmp("OpenLevel", propertyName) == 0)
        {
            int64 argument;
            (void)TypeConversionHelpers::GetAllJoynMessageArg(propertyValue, "x", &argument);
            OpenLevelChanged(this, (Platform::Object^)argument);
        }
    }
}

int32 openlevelConsumer::JoinSession(_In_ AllJoynServiceInfo^ serviceInfo, const std::string& ifName)
{
    alljoyn_sessionlistener_callbacks callbacks =
    {
        AllJoynHelpers::SessionLostHandler<openlevelConsumer>,
        AllJoynHelpers::SessionMemberAddedHandler<openlevelConsumer>,
        AllJoynHelpers::SessionMemberRemovedHandler<openlevelConsumer>
    };
    ...
    RETURN_IF_QSTATUS_ERROR(alljoyn_proxybusobject_registerpropertieschangedlistener(
        ...
        AllJoynHelpers::PropertyChangedHandler<openlevelConsumer>, 
        m_weak));
