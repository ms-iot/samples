...
openlevelWatcher::openlevelWatcher(AllJoynBusAttachment^ busAttachment) :
...
openlevelWatcher::~openlevelWatcher()
{
    UnregisterFromBus();
}

void openlevelWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.microsoft.OICBridge.*oic.r.openlevel" };
...
void openlevelWatcher::OnAnnounce(
    _In_ PCSTR name,
                        ...
                        if (0 < nbOfInterfaces)
                        {
                            ...
                            if (nullptr != arrayOfInterfaces)
                            {
                                if (alljoyn_aboutobjectdescription_getinterfaces(objectDescription, arrayOfPath.get()[iPath], arrayOfInterfaces.get(), nbOfInterfaces) > 0)
                                {
                                    ...
                                    {
                                        if (string(arrayOfInterfaces.get()[iBusInterface]).find("oic.r.openlevel") != string::npos)
                                        {
...
void openlevelWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<openlevelWatcher>
        };
...
void openlevelWatcher::Start()
{
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &openlevelWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void openlevelWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void openlevelWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
