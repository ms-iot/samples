...
ZoneManagerWatcher::ZoneManagerWatcher(AllJoynBusAttachment^ busAttachment) :
...
ZoneManagerWatcher::~ZoneManagerWatcher()
{
    UnregisterFromBus();
}

void ZoneManagerWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "net.allplay.ZoneManager" };
...
void ZoneManagerWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "net.allplay.ZoneManager"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "net.allplay.ZoneManager"),
            port);
        Added(this, args);
    }
...
void ZoneManagerWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<ZoneManagerWatcher>
        };
...
void ZoneManagerWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_ZoneManagerIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &ZoneManagerWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void ZoneManagerWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void ZoneManagerWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
