...
void GpioOneWire::DhtSensor::Init (GpioPin^ InputPin, GpioPin^ OutputPin)
{
    ...
    InputPin->CreateInterruptBuffer(43);
...
HRESULT GpioOneWire::DhtSensor::SampleInterrupts (GpioOneWire::DhtSensorReading& Reading)
{
    ...
    this->inputPin->StartInterruptBuffer(GpioPinEdge::FallingEdge);

    // Ensure that interrupt buffer always gets stopped when this function
    // exits
    struct _StopInterruptBuffer {
        ~_StopInterruptBuffer ()
        {
            this->pin->StopInterruptBuffer();
        }

        GpioPin^ pin;
    } stopInterruptBuffer = {this->inputPin};

    GpioInterruptBuffer^ buffer = this->inputPin->InterruptBuffer;
    buffer->Clear();
    ...
    auto operation = buffer->WaitForItemsAsync(43);
    operation->Completed = ref new AsyncOperationCompletedHandler<int>([&] (
        IAsyncOperation<int>^ /*AsyncInfo*/,
        AsyncStatus Status
        )
    {
    ...
    if (WaitForSingleObject(completedEvent.Get(), 20) != WAIT_OBJECT_0) {
    ...
    TimeSpan first = buffer->Pop().RelativeTime;
    for (int i = 0; i < 40; ++i)
    {
        TimeSpan second = buffer->Pop().RelativeTime;
